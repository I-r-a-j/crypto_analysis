...
def load_model(symbol):
    model_name = f"{symbol.split('-')[0]}_model.pkl"
    try:
...
# Add this function to prepare data for prediction
def prepare_data_for_prediction(data):
    # Assuming the model expects the last 30 days of 'Close' prices
    # You may need to adjust this based on your actual model input requirements
    return data['Close'].tail(5).values.reshape(1, -1)
...
selected_model = load_model(selected_symbol)

if selected_model is not None:
    try:
        input_data = prepare_data_for_prediction(data)
        prediction = selected_model.predict(input_data)
        st.write(f"Predicted price for next day: ${prediction[0]:.2f}")
    except Exception as e:
        st.error(f"Error making prediction: {str(e)}")
else:
    st.write("No prediction available for the selected cryptocurrency.")
