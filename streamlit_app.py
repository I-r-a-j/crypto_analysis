...
def main():
    ...
    if indicator in ['Moving Average', 'RSI', 'Bollinger Bands']:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            params['fast_window'] = st.slider('Fast EMA Window', 8, 20, 12)
        with col2:
            params['slow_window'] = st.slider('Slow EMA Window', 20, 30, 26)
        with col3:
            params['signal_window'] = st.slider('Signal Window', 5, 15, 9)
